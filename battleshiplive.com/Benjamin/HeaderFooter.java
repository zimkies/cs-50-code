...
public class HeaderFooter extends PdfPageEventHelper {
	...
	/**
	 *  The table that will be added as the header of the document.
	 */
	 ...
	 */
	public HeaderFooter(ServletContext context, BettinaNetwork network, String folder) {
		try {
			//float[] widths1 = {1f,2f,1f};
			
			header = new PdfPTable(1); //widths1);
			header.getDefaultCell().setBorder(PdfPCell.NO_BORDER);
			//header.addCell("");
					
			StringBuffer imageBuffer = new StringBuffer(folder);
			imageBuffer.append("/letterhead6x.gif");
	...
	public void onEndPage(PdfWriter writer, Document document) {
		try {
			...
			//header.writeSelectedRows(0, -1, document.leftMargin(), page.height() - document.topMargin() + header.getTotalHeight(),
